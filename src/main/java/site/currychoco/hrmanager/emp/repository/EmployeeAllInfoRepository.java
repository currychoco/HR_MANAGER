...
public interface EmployeeAllInfoRepository extends JpaRepository<EmployeeAllInfo, Long> {
    @Query(value="select e.emp_no, e.emp_name, e.emp_name_en, e.dept_code, d.dept_name, d.dept_name_en, e.job_code, j.job_name, j.job_name_en, e.position_code, p.position_name, p.position_name_en, e.gender, e.email, e.phone, e.start_date, a.id " +
    ...
    @Query(value="select e.emp_no, e.emp_name, e.emp_name_en, e.dept_code, d.dept_name, d.dept_name_en, e.job_code, j.job_name, j.job_name_en, e.position_code, p.position_name, p.position_name_en, e.gender, e.email, e.phone, e.start_date, a.id " +
            ...
            " left outer join `position` p on e.position_code = p.position_code " +
            "left outer join `account` a on e.emp_no = a.emp_no where e.emp_name like CONCAT('%',:name,'%')", nativeQuery = true)
